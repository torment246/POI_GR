#!/usr/bin/env python3
"""Evaluate GNPR with unconstrained beam search and frozen ID lookup."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "按 GNPR 论文直接生成三段或四段 identifier；无 Trie、无地理剪枝，"
            "无效 ID 保留在原始 Beam 排名中。"
        )
    )
...
def load_json(path: Path, name: str) -> dict[str, Any]:
    if not path.is_file():
        raise GnprEvalError(f"{name} 不存在：{path}")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise GnprEvalError(f"{name} JSON 非法") from error
    if not isinstance(value, dict):
        raise GnprEvalError(f"{name} 必须是 object")
    return value
...
def validate_checkpoints(
    ...
    *,
    ...
    if mapping_payload.get("schema_version") != "gnpr-vocab-v1":
    ...
    for checkpoint, expected_step, expected_epoch in zip(
        ...
        if checkpoint.name != f"checkpoint-{expected_step}":
            raise GnprEvalError(f"Checkpoint 顺序或步数错误：{checkpoint.name}")
        required = (
            "model.safetensors",
            "config.json",
            "tokenizer.json",
        ...
        if missing:
            raise GnprEvalError(f"{checkpoint.name} 缺少文件：{', '.join(missing)}")
        ...
        if config.get("vocab_size") != expected_vocab:
            raise GnprEvalError(f"{checkpoint.name} vocab_size 不一致")
        ...
        for token, token_id in token_mapping.items():
            if added.get(token) != token_id:
                raise GnprEvalError(f"{checkpoint.name} Token ID 不一致：{token}")
        eval_loss = next(
            (
        ...
        if eval_loss is None:
            raise GnprEvalError(f"{checkpoint.name} 缺少完整 Validation Loss")
        results.append(
            {
                "path": str(checkpoint),
    ...
    if len({item["tokenizer_json_sha256"] for item in results}) != 1:
        raise GnprEvalError("三个 GNPR checkpoint 的 tokenizer 不一致")
    return results
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            try:
                ...
                raise GnprEvalError(f"评测子集第 {line_number} 行 JSON 非法") from error
            if not isinstance(value, dict):
                raise GnprEvalError(f"评测子集第 {line_number} 行不是 object")
    ...
    *,
    ...
    for example in examples:
        ...
        if row < 0 or index.poi_id(row) != example.target_poi_id:
            raise GnprEvalError(f"Target identifier 映射不一致：{example.sample_id}")
    ...
    for start in range(0, len(examples), batch_size):
        ...
        with torch.inference_mode():
            generated = model.generate(
                ...
                max_new_tokens=tokens.max_sequence_length,
                length_penalty=1.0,
                early_stopping=True,
        ...
        if generated.sequences_scores is None:
            raise GnprEvalError("Beam Search 缺少 sequences_scores")
        ...
        if len(raw_sequences) != expected or len(raw_scores) != expected:
            raise GnprEvalError("Beam Search 返回候选数量不一致")
...
def evaluate_checkpoint(
    *,
    ...
    config = {
        "schema_version": "gnpr-generative-eval-v1",
    ...
    if progress_path.is_file():
        progress = load_json(progress_path, "GNPR progress")
        if progress.get("signature") != signature:
            raise GnprEvalError(f"已有运行配置变化：{run_dir}")
    else:
        progress = {
            "schema_version": "gnpr-eval-progress-v1",
            "status": "running",
            "signature": signature,
            "config": config,
            "next_line": 0,
    ...
    while int(progress["next_line"]) < target_rows:
        ...
        while True:
            try:
                ...
                if not smaller:
                    raise GnprEvalError("GNPR batch_size=2 仍然 OOM") from error
                batch_size = smaller[0]
                print(
                    f"CUDA OOM，当前分片改用 batch_size={batch_size}",
                    file=sys.stderr,
                )
    ...
    result = {
        "schema_version": "gnpr-eval-result-v1",
        "status": "completed",
        "config": config,
...
def main() -> int:
    ...
    try:
        ...
        if args.chunk_size <= 0:
            raise GnprEvalError("--chunk-size 必须为正整数")
        if args.smoke_limit is not None and not 1 <= args.smoke_limit <= 100:
            raise GnprEvalError("--smoke-limit 必须位于 [1, 100]")
        ...
        if subset.row_count != 10_000:
            raise GnprEvalError("固定 Validation 子集必须为 10,000 条")
        ...
        for record in records[:100]:
            ...
            row = index.lookup(example.target_codes)
            if row < 0 or index.poi_id(row) != example.target_poi_id:
                raise GnprEvalError(f"Prompt 样本目标映射失败：{example.sample_id}")
        ...
        payload = {
            "schema_version": "gnpr-valid-checkpoint-results-v1",
            "status": "completed",
            "paper_protocol": {
                "decoding": "direct generation without constrained decoding",
                "comparison_beam_size": 10,
                "identifier_lookup": "frozen conditional-length GNPR ID to POI table",
        ...
        print(json.dumps(payload, ensure_ascii=False, indent=2))
        return 0
    except (GnprEvalError, OSError, ValueError, KeyError) as error:
        print(f"GNPR 评测失败：{error}", file=sys.stderr)
        return 2
