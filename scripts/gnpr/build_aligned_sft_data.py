#!/usr/bin/env python3
"""Build active GNPR SFT by replacing identifiers in frozen TIGER samples."""
...
from poi_gr.methods.gnpr.aligned_data import build_aligned_sft_data


def main() -> int:
    parser = argparse.ArgumentParser(description="逐条替换 TIGER SID，保留用户哈希、Query、GID、历史及切分。")
    for name in ("source-dir", "tiger-id-dir", "gnpr-id-dir", "output-dir"):
    ...
    try:
        ...
        print(f"GNPR 对齐数据构建失败：{error}", file=sys.stderr)
        return 2
