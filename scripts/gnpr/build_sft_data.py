#!/usr/bin/env python3
"""Build map-search-adapted GNPR SFT data from 003 history orders."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "将 003 中的历史 Query、请求 GID 和 GNPR identifier 线性化为 "
            "Messages 数据；时间与用户 ID 仅用于校验或完全忽略，不进入 Prompt。"
        )
    )
    parser.add_argument(
        "--orders-dir",
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "正式 GNPR identifier 目录，包含 gnpr_id_manifest.json、"
            "gnpr_ids.npy 和 POI ID 文件。"
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def main() -> int:
    ...
    try:
        ...
        result = build_gnpr_sft_data(
            ...
            _from_project_root(args.gnpr_id_dir),
    ...
    except (GnprDataError, OSError, ValueError) as error:
        print(f"GNPR SFT 数据构建失败：{error}", file=sys.stderr)
        return 2
