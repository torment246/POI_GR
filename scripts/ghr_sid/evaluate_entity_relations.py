#!/usr/bin/env python3
"""Run G6 -> coarse R3 -> fine entity relation structural evaluation."""
...
DEFAULT_EXPERIMENT_ID = "EXP-20260814-07"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "复用冻结 G6 和粗关系 R3，不再追加细地理；在 post-R3 组内用"
            "数字、复合编号、类别、道路、父子实体及其共享位置字符关系"
            "编译最短可区分路径。"
        )
    )
...
def main() -> int:
    ...
    try:
        if args.validate_only:
            payload = validate_entity_structure_output(output_dir)
        else:
            result = evaluate_entity_relation_structure(
        ...
        print(f"GHR-SID 细实体关系实验失败：{error}", file=sys.stderr)
        return 2
