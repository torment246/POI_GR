...
class GenerativeEvalTest(unittest.TestCase):
    ...
    def test_evaluation_fingerprint_sources_follow_reorganized_modules(self) -> None:
        evaluator_source = PROJECT_ROOT / "src" / "poi_gr" / "sft" / "evaluation.py"
        trie_source = PROJECT_ROOT / "src" / "poi_gr" / "pid" / "trie.py"
        self.assertTrue(evaluator_source.is_file())
        self.assertTrue(trie_source.is_file())
        source = evaluator_source.read_text(encoding="utf-8")
        self.assertIn('"sft" / "evaluation.py"', source)
        self.assertIn('"pid" / "trie.py"', source)
        self.assertNotIn('"poi_gr" / "generative_eval.py"', source)
        self.assertNotIn('"poi_gr" / "pid_trie.py"', source)
