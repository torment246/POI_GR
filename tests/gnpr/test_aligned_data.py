"""Synthetic checks for active GNPR's paired user/hash/history contract."""
...
from poi_gr.methods.gnpr.aligned_data import aligned_special_tokens, build_aligned_sft_data, transform_record
from poi_gr.methods.gnpr.data import GnprDataError, GnprIdLookup
...
class AlignedDataTest(unittest.TestCase):
    ...
    def test_bad_uid_target_history_or_split_fail_closed(self):
        ...
        for key, value in (("user_token", "<U_0001>"), ("target_poi_id", "200"),
            ...
            source = record()
            source[key] = value
            with self.assertRaises(GnprDataError):
    ...
    def test_full_streaming_builder_checks_hashes_and_preserves_protocol(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            gnpr = GnprIdLookup({100: 0}, np.array([[9, 8, 7, -1]], dtype=np.int32), 1, (512, 512, 512, 1))
            with patch("poi_gr.methods.gnpr.aligned_data.load_tiger_id_lookup", return_value=(tiger, {}, "tmap", "tmanifest")), \
                 patch("poi_gr.methods.gnpr.aligned_data.load_gnpr_id_lookup", return_value=(gnpr, {"schema_version": "gnpr-poi-identifier-v1"}, "gmap", "gmanifest")):
                result = build_aligned_sft_data(source_dir=source, tiger_id_dir=root,
                    gnpr_id_dir=root, output_dir=root / "output", progress=lambda _: None)
                ...
                with self.assertRaisesRegex(GnprDataError, "拒绝覆盖"):
                    build_aligned_sft_data(source_dir=source, tiger_id_dir=root,
                        gnpr_id_dir=root, output_dir=root / "output")
                outputs["train.jsonl"]["sha256"] = "wrong"
                (source / "manifest.json").write_text(json.dumps(manifest))
                with self.assertRaisesRegex(GnprDataError, "SHA256"):
                    build_aligned_sft_data(source_dir=source, tiger_id_dir=root,
                        gnpr_id_dir=root, output_dir=root / "bad-output", progress=lambda _: None)
                self.assertFalse((root / "bad-output").exists())

    def test_missing_user_vocab_is_not_silently_dropped(self):
        with self.assertRaises(GnprDataError):
            aligned_special_tokens((512, 512, 512, 2), {"user_tokens": []})
