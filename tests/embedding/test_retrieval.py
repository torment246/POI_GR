...
from scripts.embedding.evaluate_retrieval import (
    EvaluationError,
...
class EmbeddingRetrievalMetricsTest(unittest.TestCase):
    def test_new_protocol_uses_isolated_output_root(self) -> None:
        query_path, mapping_path, output_dir = resolve_output_paths(
            {"output_root": "outputs/evaluation/protocol_v2"},
            "bge_m3",
            "none",
        )

        self.assertEqual(query_path.name, "bge_m3_no_instruction.npy")
        self.assertEqual(mapping_path.name, "bge_m3_no_instruction_rows.jsonl")
        self.assertEqual(output_dir.name, "bge_m3_no_instruction")
        self.assertIn("protocol_v2", str(output_dir))
