...
Qwen3-Embedding-0.6B 和 Qwen3-Embedding-4B 的北京 POI 全量向量均已完成并通过校验。两者无 Instruction 的 10,000 条订单 Faiss GPU 精确召回对照已完成：0.6B 的 Hit@1/10/20 为 12.03%/28.25%/34.51%，4B 为 9.72%/21.18%/25.12%，暂不自动选择模型。0.6B 向量的首轮全量 RQ-VAE 实验也已完成，当前等待用户核验向量召回对照和 SID 重建—碰撞取舍，再决定下一项实验。
