"""Build deterministic order-to-Final-PID SFT data."""
...
from .dedup_pid import sha256_file
from .geohash_pid import (
    GEOHASH_ALPHABET,
    GeohashPidError,
    encode_geohash,
    tokens_to_geohash,
...
class SftMainDataError(ValueError):
    pass
...
class TimeSplit:
    ...
    def __post_init__(self) -> None:
        if self.train_start > self.train_end:
            raise SftMainDataError("train_start 不能晚于 train_end")
    ...
    def split_for(self, value: date) -> str:
        ...
        if value == self.test_date:
            return "test"
        raise SftMainDataError(f"日期 {value.isoformat()} 不属于任何切分")
...
def parse_iso_date(value: str, name: str) -> date:
    ...
    try:
        ...
        raise SftMainDataError(f"{name} 必须为 YYYY-MM-DD：{value}") from error
    if parsed.isoformat() != value:
        raise SftMainDataError(f"{name} 必须为 YYYY-MM-DD：{value}")
    return parsed
...
def discover_order_files(orders_dir: Path) -> tuple[Path, ...]:
    ...
    if not orders_dir.is_dir():
        raise SftMainDataError(f"订单目录不存在：{orders_dir}")
    files = tuple(
        sorted(
            path
    ...
    if not files:
        raise SftMainDataError(f"订单目录中没有 part-* 分片：{orders_dir}")
    unexpected = {
        path.name
        for path in orders_dir.iterdir()
...
def _load_json_object(path: Path, name: str) -> dict[str, Any]:
    if not path.is_file():
        raise SftMainDataError(f"{name} 不存在：{path}")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise SftMainDataError(f"{name} JSON 解析失败：{path}") from error
    if not isinstance(value, dict):
        raise SftMainDataError(f"{name} 必须是 JSON object：{path}")
    return value
...
def load_pid_lookup(
    ...
    if manifest.get("schema_version") != "dedup-pid-v1":
        raise SftMainDataError("PID manifest schema_version 必须是 dedup-pid-v1")
    if manifest.get("status") != "completed":
        raise SftMainDataError("PID manifest 状态不是 completed")
    poi_count = manifest.get("poi_count")
    if isinstance(poi_count, bool) or not isinstance(poi_count, int):
        raise SftMainDataError("PID manifest poi_count 无效")
    mapping_spec = manifest.get("outputs", {}).get(
        "poi_pid_mapping.parquet"
    )
    if not isinstance(mapping_spec, dict):
        raise SftMainDataError("PID manifest 缺少 Parquet 映射定义")
    mapping_sha256 = sha256_file(pid_mapping_path)
    if mapping_spec.get("sha256") != mapping_sha256:
        raise SftMainDataError("PID 映射 SHA256 与 manifest 不一致")

    parquet_file = pq.ParquetFile(pid_mapping_path)
    if parquet_file.metadata.num_rows != poi_count:
        raise SftMainDataError("PID 映射行数与 manifest poi_count 不一致")
    ...
    for row_group_index in range(parquet_file.num_row_groups):
        ...
        if not np.array_equal(
            ...
            raise SftMainDataError("PID 映射 final_pid_length 与 Dedup 状态不一致")
        if not np.array_equal(group_requires, group_dedup >= 0):
            raise SftMainDataError("PID 映射 requires_dedup 与 dedup_code 不一致")

        for local_index, poi_id in enumerate(data["poi_id"]):
            if not isinstance(poi_id, str) or not poi_id:
                raise SftMainDataError("PID 映射包含非法 poi_id")
            if poi_id in row_by_poi_id:
                raise SftMainDataError(f"PID 映射包含重复 poi_id：{poi_id}")
            ...
            if data["final_pid_key"][local_index] != expected_key:
                raise SftMainDataError("PID 映射 final_pid_key 与 Token 不一致")
        offset = end
    if offset != poi_count or len(row_by_poi_id) != poi_count:
        raise SftMainDataError("PID 映射行数或 POI 唯一数不完整")
    return (
        PidLookup(
